...
class CreatScrollableFrame(ctk.CTkScrollableFrame):
    def __init__(self, Fenetre, width:int=300, height:int=300, bg_color='transparent',
        ...
        # Initialisation de la classe CTkScrollableFrame
        super().__init__(
            master=Fenetre,
            width=width,
            height=height,
    ...
    # ➕ Méthodes utilitaires pour la gestion du layout

    def ScrollableFramePack(self, padx=0, pady=0, fill="both", expand=True, **kwargs):
        self.pack(padx=padx, pady=pady, fill=fill, expand=expand, **kwargs)

    def ScrollableFramePlace(self, relx=None, rely=None, anchor=None, **kwargs):
    ...
    def ScrollableFrameGrid(self, row=0, column=0, padx=0, pady=0, sticky="nsew", rw=1, cw=1, **kwargs):
